"""
SSH key-related database operations.
...
class SSHKeyDatabaseMixin:
    """Mixin class containing SSH key-related database operations."""
    ...
    def get_all_ssh_keys(self) -> List[Dict[str, Any]]:
        """Get all SSH keys in the database."""
        with self.get_cursor() as cursor:
            cursor.execute("SELECT * FROM ssh_keys ORDER BY registered_at DESC")
            return [dict(row) for row in cursor.fetchall()]

    def get_users_with_keys(self) -> List[str]:
        """Get list of usernames that have registered SSH keys."""
        with self.get_cursor() as cursor:
    ...
    def get_key_owner(self, public_key: str) -> Optional[str]:
        """Get the username that owns a specific public key."""
        with self.get_cursor() as cursor:
            cursor.execute("""
    ...
    def is_key_registered_elsewhere(self, username: str, public_key: str) -> bool:
        """Check if a public key is registered to a different user."""
        owner = self.get_key_owner(public_key)
