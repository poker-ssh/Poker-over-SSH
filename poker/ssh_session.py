"""
SSH session management for Poker-over-SSH.
Handles individual SSH sessions with room awareness.
"""
...
if asyncssh:
    class PokerSSHSession(RoomSession, asyncssh.SSHServerSession):
        """SSH Session with asyncssh inheritance when available."""
    ...
    PokerSSHSession = RoomSession  # type: ignore


def create_ssh_session(stdin, stdout, stderr, server_state=None, username=None):
    """Create and return an SSH session instance."""
    return PokerSSHSession(stdin, stdout, stderr, server_state=server_state, username=username)
